...
**Sonar AI CodeFix** est une fonctionnalité intégrée à SonarQube qui utilise un grand modèle de langage (LLM) pour **proposer automatiquement une correction de code** sur les issues détectées par l'analyse Sonar.
...
- ✅ Disponible dans l'**interface web** et dans votre **IDE**

### Ce que ne fait pas AI CodeFix

- ❌ Ne modifie pas le comportement fonctionnel du code
- ❌ Ne résout pas toutes les issues (couverture partielle par langage)
...
![Issues List](https://docs.sonarsource.com/sonarqube-server/instance-administration/ai-features/img/ai-codefix-step1.png)

> Filtrez par sévérité, type ou langage pour trouver les issues les plus critiques à traiter.

### Étape 2 — Identifier une issue éligible à AI CodeFix

Les issues éligibles affichent un **badge AI** ou un bouton **Generate AI Fix** lorsque vous ouvrez le détail de l'issue.
...
2. SonarQube envoie le contexte de l'issue au LLM configuré
...
- Cliquez **Apply** pour accepter la correction → SonarQube vous propose d'**ouvrir dans l'IDE** ou de copier le patch
...
| IDE | Plugin requis |
|-----|---------------|
| VS Code | SonarQube for VS Code (extension officielle) |
| IntelliJ IDEA / WebStorm / ... | SonarQube for IntelliJ (plugin officiel) |

Le plugin doit être configuré en **mode connecté** (Connected Mode) à votre instance SonarQube Data Center.

### Parcours dans VS Code

```
1. Ouvrir le fichier concerné dans VS Code
2. Les issues Sonar apparaissent en surbrillance (ondulé)
3. Survoler l'issue → panneau SonarQube s'ouvre
4. Cliquer "Generate AI Fix" dans le panneau
5. Réviser le diff proposé dans l'éditeur
6. Cliquer "Apply Fix" pour intégrer le correctif
```
...
3. Alt+Entrée sur la ligne concernée → menu contextuel Sonar
4. Choisir "SonarQube: Generate AI Fix"
5. Réviser le diff dans la fenêtre "Sonar AI Fix Preview"
6. Valider avec "Apply"
```
...
│  (contexte du fichier)   │ +  │  Sonar (ex: S1117)           │
│                          │    │  + examples good/bad         │
└──────────────────────────┘    └──────────────────────────────┘
                   ↓
         LLM génère un patch minimal
                   ↓
...
AI CodeFix est disponible sur une **sélection de règles certifiées** par Sonar pour les langages suivants :
...
> La liste complète des règles éligibles par langage est disponible dans la [documentation officielle Sonar](https://docs.sonarsource.com/sonarqube-server/2025.2/user-guide/rules/rules-for-ai-codefix).

### Pourquoi le bouton "Generate AI Fix" n'apparaît-il pas sur certaines issues ?

Deux raisons possibles :
1. La **règle** de cette issue n'est pas encore couverte par AI CodeFix
...
Cela dépend de la configuration de votre administrateur. En mode Data Center, l'admin peut avoir configuré un LLM auto-hébergé (Ollama, LiteLLM, vLLM) auquel cas le code ne quitte pas votre réseau. Demandez à votre admin quelle configuration est en place.
...
| Ressource | Lien |
|-----------|------|
| Documentation officielle AI CodeFix | [docs.sonarsource.com](https://docs.sonarsource.com/sonarqube-server/ai-capabilities/ai-codefix) |
| Règles éligibles par langage | [Rules for AI CodeFix](https://docs.sonarsource.com/sonarqube-server/2025.2/user-guide/rules/rules-for-ai-codefix) |
| Plugin SonarQube for VS Code | [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=SonarSource.sonarlint-vscode) |
| Plugin SonarQube for IntelliJ | [JetBrains Marketplace](https://plugins.jetbrains.com/plugin/7973-sonarlint) |
